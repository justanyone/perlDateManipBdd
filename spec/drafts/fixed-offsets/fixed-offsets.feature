...
Feature: Parse selected fixed UTC-offset civil times
  ...
  Scalar date text uses the six-field serialization YYYYMMDDHH:MM:SS.
  ...
  Scenario Outline: Preserve the supplied civil fields and expose UTC for a supported offset in <case>
    ...
    Then the parsed-zone scalar is "2040022912:34:56"
    And the UTC scalar is "<utc_scalar>"
    And formatting as local date, time, numeric offset, and zone text is "<formatted>"
    And the parse error is empty

    Examples:
      | case                 | offset      | utc_scalar       | formatted                            |
  ...
  @FO-REJECT-SECONDS
  Scenario: Report the observed failure for a generated seconds offset input in FO-REJECT-SECONDS
