...
Feature: Replacing a date carrier's zone or zoned civil date
  ...
  Background:
    ...
    And a valid receiver is read in native scalar and six-field contexts before replacement
  ...
  Scenario: Documented zone requests preserve wall fields and choose the expected instant
    ...
    And the result list has six fields matching the normalized wall result
    And the local result equals the GMT result because the fixed local zone is UTC
    And the exact results are:
      | case | initial receiver | exact request | receiver | zone request | daylight request | wall result | GMT result |
  ...
  Scenario: Undocumented zone option values are accepted as compatibility behavior
    ...
    Then every status is 0 with every call and observer error empty, no exception, and no warning
    And the exact results are:
      | case | initial receiver | exact request | receiver | zone request | daylight request | wall result | GMT result |
  ...
  Scenario: Zone arity, receiver-state, and gap failures clear or retain state as observed
    ...
    Then the scalar, local, and GMT results are defined empty text and the list result is an empty list
    And the first scalar observer changes the recorded error to "[value] Object does not contain a date"
    ...
    And none of these calls raises an exception

  @reference-binding @observed-compatibility @disputed
  Scenario: Offset and unresolved zone inputs raise a Date-Manip 7.00 binding exception
    When I make the zone request in each row
    Then the call status is undefined and its error before and after is empty
    And the exception begins "Can't locate object method \"__zone\" via package \"Date::Manip::Base\""
    ...
    And the affected requests are:
      | case | initial receiver | exact request | receiver | zone input |
      | DSET-ZONE-003-OFFSET-TEXT | valid "2040-02-29 16:05:09 America/New_York" | selector text "zone"; arguments [text "+05:30"] | 2040-02-29 16:05:09 America/New_York | text offset +05:30 |
      | DSET-ZONE-004-OFFSET-LIST | valid "2040-02-29 16:05:09 America/New_York" | selector text "zone"; arguments [list [number 5, number 30, number 0]] | 2040-02-29 16:05:09 America/New_York | offset fields 5, 30, 0 |
      | DSET-ZONE-009-INVALID-ZONE | valid "2040-02-29 16:05:09 America/New_York" | selector text "zone"; arguments [text "Not/A_Zone"] | 2040-02-29 16:05:09 America/New_York | Not/A_Zone |
      | DSET-ZONE-020-TWO-AS-SINGLE-ARG | valid "2040-02-29 16:05:09 Etc/UTC" | selector text "zone"; arguments [number 2] | 2040-02-29 16:05:09 Etc/UTC | invalid single numeric 2 |
  ...
  Scenario: Whole zoned-date requests initialize, replace, recover, and disambiguate
    ...
    Then every status is 0, every call and observer error is empty, and no exception or warning occurs
    And every scalar is defined, every list has six exact fields, and local equals GMT
    And the exact results are:
      | case | initial receiver | exact request | receiver | zone | replacement | daylight request | wall result | GMT result |
  ...
  @reference-binding @observed-compatibility @disputed
  Scenario: Zoned-date offsets and unresolved names raise the same binding exception
    When I replace a complete zoned date using each zone input
    Then the call status is undefined, its error before and after stays empty, and no warning occurs
    And the exception begins "Can't locate object method \"__zone\" via package \"Date::Manip::Base\""
    And the interrupted mutation leaves defined empty scalar, local, and GMT text and an empty list
    And the first scalar observer changes the error to "[value] Object does not contain a date"
    And later list, local, and GMT observers retain that value error
    And the affected requests are:
      | case | initial receiver | exact request | receiver | zone input | replacement |
      | DSET-ZDATE-003-OFFSET-LIST | unset | selector text "zdate"; arguments [list [number -5, number -30, number 0], list [number 2041, number 1, number 2, number 3, number 4, number 5]] | unset | offset fields -5, -30, 0 | 2041-01-02 03:04:05 |
      | DSET-ZDATE-010-INVALID-ZONE | valid "2040-02-29 16:05:09 Etc/UTC" | selector text "zdate"; arguments [text "Not/A_Zone", list [number 2041, number 1, number 2, number 3, number 4, number 5]] | valid UTC date | Not/A_Zone | 2041-01-02 03:04:05 |
      | DSET-ZDATE-015-OFFSET-TEXT | unset | selector text "zdate"; arguments [text "-05:30", list [number 2041, number 1, number 2, number 3, number 4, number 5]] | unset | text offset -05:30 | 2041-01-02 03:04:05 |
