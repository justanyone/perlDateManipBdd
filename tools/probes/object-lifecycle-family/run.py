#!/usr/bin/env python3
"""Run each public object-lifecycle probe twice in fresh clean processes."""
...
FIXTURE_PATH = ROOT / "docs/research/object-lifecycle-family/cases.json"
FIXTURE_BYTES = FIXTURE_PATH.read_bytes()
FIXTURE_SHA256 = hashlib.sha256(FIXTURE_BYTES).hexdigest()
FIXTURES = json.loads(FIXTURE_BYTES.decode("utf-8"))
PROBE = ROOT / "tools/probes/object-lifecycle-family/probe.pl"
...
def invoke(case_id):
    with tempfile.TemporaryDirectory(prefix="date-manip-object-") as cwd:
...
def main():
    ...
    print(json.dumps({
        ...
        "command": "python3 tools/probes/object-lifecycle-family/run.py",
        "repetitions": 2,
        "timeout_seconds": 15,
        "parallel_workers": 4,
        "clean_environment_keys": sorted(ENV),
        "fresh_temporary_working_directory_per_attempt": True,
        "fixture_file": "docs/research/object-lifecycle-family/cases.json",
        "fixture_sha256": FIXTURE_SHA256,
        "sha256": {
            str(path.relative_to(ROOT)): hashlib.sha256(path.read_bytes()).hexdigest()
            for path in [PROBE, pathlib.Path(__file__).resolve(), FIXTURE_PATH,
                         *[ROOT / "local/date-manip-7.00/lib/perl5/Date/Manip" / name
                           for name in ("Date.pm", "Delta.pm", "Recur.pm", "Obj.pm")]]
        },
